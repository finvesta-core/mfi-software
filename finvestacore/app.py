...
def borrower_status():
    ...
    with get_db_connection() as conn:
        ...
        if search_type == 'loan_id':
            try:
                loan_id = _parse_loan_id(search_value) if 'PL' in search_value.upper() else int(search_value)
                cursor.execute("""
    ...
    # Format currency (add _format_currency if missing)
